...
export default function ShopPage() {
    ...
    // Fetch from E-Commerce Service (Port 8091)
    useEffect(() => {
        fetch("http://localhost:8091/api/products")
            .then(async (res) => {
                if (!res.ok) {
                    const text = await res.text();
                    throw new Error(`Backend blocked the request. Status: ${res.status}`);
                }
                return res.json();
            })
            .then((data) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            {/* Header Section */}
            <div className="bg-emerald-900 text-white py-12 text-center">
